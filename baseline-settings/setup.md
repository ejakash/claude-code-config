...
- `<LINUX_USER>` — your WSL username (appears in `statusLine.command` path)
...
1. If `~/.claude/settings.json` does not exist: copy this module's `settings.json` after substituting `<LINUX_USER>` with your WSL username (e.g. `sed 's|<LINUX_USER>|myuser|g' settings.json > ~/.claude/settings.json`).
